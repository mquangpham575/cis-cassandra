...
resource "azurerm_network_interface" "node" {
  ...
  resource_group_name = azurerm_resource_group.main.name
  location            = each.key == "master" ? var.location : var.db_location

  ip_configuration {
    name                          = "internal"
    subnet_id                     = each.key == "master" ? azurerm_subnet.master.id : azurerm_subnet.cassandra.id
...
resource "azurerm_network_interface_security_group_association" "node" {
  ...
  network_security_group_id = each.key == "master" ? azurerm_network_security_group.master.id : azurerm_network_security_group.cassandra.id
}
