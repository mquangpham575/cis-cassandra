...
variable "db_location" {
  description = "Azure region for the database nodes"
  type        = string
...
# ---------------------------------------------------------------------------
locals {
  master_location = var.location
  db_location     = var.db_location

  # 1 Master (Management) and 3 Database nodes
  nodes = {
    master = { ip = "10.1.1.10", role = "master", is_seed = false }
    db1    = { ip = "10.0.1.11", role = "db",     is_seed = true }
    db2    = { ip = "10.0.1.12", role = "db",     is_seed = false }
    db3    = { ip = "10.0.1.13", role = "db",     is_seed = false }
  }

  # Master and DB network ranges are split across regions and peered together
  master_vnet_cidr  = "10.1.0.0/16"
  master_subnet_cidr = "10.1.1.0/24"
  db_vnet_cidr      = "10.0.0.0/16"
