            ...
            >
              ...
              <div className="flex gap-3 text-xs w-full">
                <span className={status?.reachable ? 'text-green-400' : 'text-gray-500'}>
                  SSH: {status?.reachable ? 'online' : 'offline'}
                </span>
                <span className={status?.cassandra_running ? 'text-green-400' : 'text-gray-500'}>
                  Cassandra: {status?.cassandra_running ? 'online' : 'offline'}
                </span>
              ...
              {node && (
                <div className="flex gap-2 text-xs w-full">
                  <span className="text-green-400">Passed: {node.score.passed}</span>
                  <span className="text-red-400">Failed: {node.score.failed}</span>
                  <span className="text-yellow-400">Needs review: {node.score.needs_review}</span>
                </div>
              )}
            </div>
          )
