  ...
  return (
    ...
    >
      ...
      <div className="flex gap-3 text-xs">
        <span className="text-green-400">Passed: {report.score.passed}</span>
        <span className="text-red-400">Failed: {report.score.failed}</span>
        <span className="text-yellow-400">Needs review: {report.score.needs_review}</span>
      </div>
      ...
      {report.error && (
        <p className="mt-2 text-xs text-red-400 truncate">Warning: {report.error}</p>
      )}
    </button>
  )
}
