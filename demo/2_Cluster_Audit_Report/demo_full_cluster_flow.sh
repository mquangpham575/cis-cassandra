...
log_warn "-> Đang gây lỗi Logging Level (4.1) & TCP Keepalive (OS.4) trên Node 2 (10.0.1.12)..."
ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no "$SSH_USER@10.0.1.12" "sudo sed -i 's/<root level=\"INFO\">/<root level=\"OFF\">/g' /etc/cassandra/logback.xml && sudo sysctl -w net.ipv4.tcp_keepalive_time=7200" > /dev/null

log_warn "-> Đang gây lỗi Auditing (4.2) & File Permissions (OS.2) trên Node 3 (10.0.1.13)..."
ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no "$SSH_USER@10.0.1.13" "sudo sed -i '/^audit_logging_options:/,/enabled:/ s/enabled: true/enabled: false/' /etc/cassandra/cassandra.yaml && sudo chmod 777 /etc/cassandra/cassandra.yaml" > /dev/null
...
bash "$DIR/cis-tool.sh" audit cluster | grep -Ei "ID .*3\.6|ID .*4\.1|ID .*4\.2|ID .*OS\.2|ID .*OS\.4|ID .*OS\.7|REPORT FROM NODE" -A 2
