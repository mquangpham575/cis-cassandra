...
log_header "FLOW 2: TỐI ƯU HÓA KERNEL (VM.SWAPPINESS) TRÊN NODE 1 (10.0.1.11)"

# 1. LÀM SAI (BREAK)
log_info "BƯỚC 1: Đưa cấu hình về mặc định (vm.swappiness=60) trên Node 1..."
ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no "$SSH_USER@10.0.1.11" "sudo sysctl -w vm.swappiness=60" > /dev/null
echo -e "   -> Giá trị hiện tại trên Node 1: $(ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no "$SSH_USER@10.0.1.11" 'sysctl vm.swappiness')"

# 2. KIỂM TRA (AUDIT)
log_info "BƯỚC 2: Chạy Audit trên Node 1 để phát hiện lỗi hiệu năng..."
ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no "$SSH_USER@10.0.1.11" "sudo bash ~/cis-tool/cis-tool.sh audit --section os" | grep -Ei "ID .*6" -A 5

# 3. VÁ LỖI (HARDEN)
log_info "BƯỚC 3: Tối ưu hóa hệ thống tự động trên Node 1..."
...
# 4. XÁC NHẬN (VERIFY)
log_info "BƯỚC 4: Kiểm tra lại thông số Kernel trên Node 1..."
ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no "$SSH_USER@10.0.1.11" "sudo bash ~/cis-tool/cis-tool.sh audit --section os" | grep -Ei "ID .*6" -A 5
...
log_header "KẾT THÚC FLOW 2: NODE 1 ĐÃ TỐI ƯU & CÓ BÁO CÁO"
